...
// CODE HERE
// const evenNumbers = mixedNumbers.filter(function(arr){
//   return arr % 2 === 0
// })

let evenNumbers = mixedNumers.filter(arr => arr % 2 ===0 )
...
// CODE HERE
const postTaxPrices = postTaxPrices.map(function(arr){
  return arr * 1.07
...
// CODE HERE
const myStrongest = myStrongest.filter(monster => monster.CP > 200) // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
// CODE HERE
orders = orders.map((elem) => elem.price + (elem.price * elem.tax) )
...
// CODE HERE
let bobsTotal = purchases.filter(purchases => purchase.owner === 'Bob').reduce((a,b) => a + b.price, 0)